...
app.get("/cards/:cardId", async (c) => {
  ...
  const card = store.cards.get(id) ?? [...store.cards.values()].find((ca) => ca.nfcShortId === id) ?? null;
  ...
  return c.json({
    ...
    owner: owner
      ? { id: owner.id, displayName: owner.displayName, isAnonymous: (owner as unknown as { isAnonymous?: boolean }).isAnonymous ?? false }
      : null,
...
app.get("/cards/:cardId/3d", async (c) => {
  ...
  return c.json({
    ...
    creator: drop
      ...
      : null,
    owner: owner
      ? { id: owner.id, name: owner.displayName, link: `/u/${(owner as unknown as { username?: string }).username ?? owner.id}` }
      : null,
...
app.post(
  ...
  async (c) => {
    ...
    if (card.verifyStatus === "tamper_detected") {
      return c.json({
        verifyStatus: "tamper_detected" as const,
        message: "Tamper terdeteksi — kartu pernah dibuka",
...
app.get("/sun-verify", async (c) => {
  ...
  if (uid)
    card =
      ([...store.cards.values()].find(
        (ca) => (ca as unknown as { nfcUid: string }).nfcUid.toLowerCase() === String(uid).toLowerCase(),
      ) as never) ?? null;
  if (!card && shortId)
    card =
      ([...store.cards.values()].find((ca) => (ca as unknown as { nfcShortId: string }).nfcShortId === String(shortId)) as never) ?? null;
  if (!card) return c.json({ verifyStatus: "unknown" as const, message: "Kartu tidak terdaftar" }, 404);
  if ((card as unknown as { verifyStatus: string }).verifyStatus === "tamper_detected")
    return c.json({ verifyStatus: "tamper_detected" as const, card: { id: (card as unknown as { id: string }).id } }, 200);
  return c.json({
    verifyStatus: "verified" as const,
    card: { id: (card as unknown as { id: string }).id },
    redirectTo: `/cards/${(card as unknown as { id: string }).id}/3d`,
    verifiedBadge: "Verified Card",
  });
});
