...
app.get("/verify/:shortId", async (c) => {
 ...
 // QR/shortId verify = "registered" (verifikasi dasar, lebih lemah dari tap NFC)
...
app.post("/verify-nfc", zValidator("json", z.object({
 ...
 // Simulasi verifikasi: anggap valid kecuali status tamper
 if (card.verifyStatus === "tamper_detected") {
 return c.json({ verifyStatus: "tamper_detected" as const, message: "Tamper terdeteksi — kartu pernah dibuka", card: { id: card.id, unitNumber: card.unitNumber, variant: card.variant }, drop: store.drops.get(card.dropId) });
 }
