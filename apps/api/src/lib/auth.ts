...
import { authHeaderToToken, ensureSeed, getUserByToken, store, type User } from "./store.js";
import { getSupabase } from "./supabase.js";

// Supabase Auth JWT middleware (docs/10 §3.3).
// - Production: verify Supabase JWT (JWKS), sub = users.id (uuid).
// - Dev fallback (SUPABASE_URL unset): in-memory demo session, so local demo flows keep working.
 ...
 * 1. Supabase configured -> verify JWT, load users row by sub (401 invalid, 403 suspended).
 * 2. No Supabase (dev/demo) -> in-memory session lookup.
 */
