...
export default function Wallet(){
  ...
  async function onTopup(){
    try{
      ...
      push(`Top-up ${amount} C-Coin berhasil. Saldo: ${r.wallet.balanceCCoin} C-Coin.`, "success");
      refetch();
    }catch(e:any){ push(e.message || String(e), "error"); }
  }
  async function onPayout(){
    try{ const r=await api.payout(payoutAmt); push(`Payout ${payoutAmt} C → ${r.netCCoin} C (${formatIdr(r.netIdr)}), biaya ${r.feeCCoin} C`, "success"); refetch(); }
    catch(e:any){ push(e.message || String(e), "error"); }
  }
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:18}}>
    <div>
      ...
      <h1 className="h2">C-Coin</h1>
      <p className="muted">1 C-Coin = Rp 10.000</p>
    </div>

    <div className="grid-2">
      <div className="card card-pad" style={{background:"linear-gradient(135deg,#1a1a2e 0%,#2a2030 100%)"}}>
        <div style={{fontSize:11,fontWeight:700,letterSpacing:"0.08em",color:"var(--gold)",textTransform:"uppercase"}}>Saldo</div>
        ...
        <div style={{fontSize:13,color:"var(--muted)"}}>≈ {formatIdr(w.balanceIdrEquiv ?? w.balanceCCoin*rate)}</div>
      </div>

      <div className="card card-pad" style={{display:"flex",flexDirection:"column",gap:12}}>
        <div style={{fontWeight:700}}>Isi Saldo</div>
        ...
        <button className="btn-gold" onClick={onTopup} style={{padding:"10px"}}>Isi {amount} C-Coin</button>
        <div style={{borderTop:"1px solid var(--border)",marginTop:4,paddingTop:12,display:"flex",flexDirection:"column",gap:8}}>
          <div style={{fontWeight:700,fontSize:13}}>Tarik ke Rekening</div>
          <div style={{display:"flex",gap:8}}>
            <input className="input" type="number" min={10} value={payoutAmt} onChange={e=> setPayoutAmt(Number(e.target.value))} style={{flex:1}} placeholder="Jumlah C-Coin"/>
            <button className="btn-ghost" onClick={onPayout}>Tarik</button>
          </div>
          <div style={{fontSize:11,color:"var(--dim)"}}>Minimal 10 C-Coin. Biaya 1%.</div>
        </div>
      </div>
    </div>
