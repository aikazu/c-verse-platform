...
export default function Privacy(){
  ...
  async function toggle(){ setSaving(true); try{ await api.patchPrivacy(!isAnonymous); push(!isAnonymous ? "Profil disembunyikan" : "Profil ditampilkan", "success"); refetch(); }catch(e:any){ push(e.message,"error"); } finally{ setSaving(false); } }
  async function toggleConsent(kind: "analytics"|"market"){
    ...
    try{
      ...
      push("Preferensi disimpan","success"); refetch();
  ...
  return <div style={{maxWidth:560, margin:"0 auto", display:"flex", flexDirection:"column", gap:18}}>
    <div>
      ...
      <p className="muted" style={{marginTop:6}}>Kontrol visibilitas koleksi & izin data</p>
    </div>
    ...
    <div className="card card-pad" style={{display:"flex", flexDirection:"column", gap:14}}>
      ...
      <p className="muted" style={{fontSize:11}}>Framework consent dibangun sejak awal (docs 09 3.4). Kreator hanya melihat data agregat/anonim bila kamu izinkan.</p>
      <label style={{display:"flex", alignItems:"center", gap:10, fontSize:13, cursor:"pointer"}}>
        <input type="checkbox" checked={ca} onChange={()=> toggleConsent("analytics")} disabled={saving} /> Izinkan kreator lihat insight anonim (visitor anonim, repeat rate) — <span style={{fontFamily:"var(--font-mono)", fontSize:11, color:"var(--text-muted)"}}>consent_analytics_detail</span>
      </label>
      <label style={{display:"flex", alignItems:"center", gap:10, fontSize:13, cursor:"pointer"}}>
        <input type="checkbox" checked={cm} onChange={()=> toggleConsent("market")} disabled={saving} /> Izinkan data agregat untuk laporan pasar — <span style={{fontFamily:"var(--font-mono)", fontSize:11, color:"var(--text-muted)"}}>consent_data_market</span>
      </label>
    </div>
  </div>;
}
