...
export default function ListingDetail(){
  ...
  if(!data) return <div className="card card-pad">Listing tidak ditemukan. <Link to="/marketplace" style={{color:"var(--gold)"}}>Marketplace →</Link></div>;
  ...
  async function onBid(){
    ...
    setBusy(true);
    try{ await api.placeBidLegacy(listing.id, amt); push("Bid "+amt+" C-Coin berhasil!","success"); setBidAmt(0); refetch(); }
  ...
  async function onAccept(){
    setBusy(true);
    try{ await api.acceptBidLegacy(listing.id); push("Lelang di-settle — ownership pindah.","success"); refetch(); }
    catch(e:any){ push(e.message,"error"); } finally{ setBusy(false); }
  }
    ...
    <div className="grid-2" style={{alignItems:"start"}}>
      <div className="card card-pad" style={{display:"flex",flexDirection:"column",gap:12}}>
        ...
        : <div style={{display:"flex",flexDirection:"column",gap:8}}>
            ...
            {isSeller && listing.currentBidCCoin && <button className="btn-ghost" onClick={onAccept} disabled={busy} style={{padding:10}}>Accept Winning Bid — Settle</button>}
            <Link to={card ? "/cards/"+card.id : "/browse"} style={{fontSize:12, color:"var(--gold)"}}>Atau bid langsung di halaman kartu (Browse) →</Link>
          </div>}
      </div>
      <div className="card">
        ...
        {bids.length===0 ? <div style={{padding:20,textAlign:"center",color:"var(--muted)",fontSize:13}}>Belum ada bid.</div> :
