...
export default function ListingDetail(){
  ...
  if(!data) return <div className="card card-pad">Listing tidak ditemukan.</div>;
  const listing:any = (data as any).listing;
  ...
  async function onBid(){
    if(!user){ push("Login dulu untuk bid","info"); return; }
    const amt = bidAmt || minBid;
    if(amt < minBid){ push(`Bid minimal ${minBid} C-Coin (5% increment)`, "error"); return; }
    setBusy(true);
    try{ await api.placeBid(listing.id, amt); push(`Bid ${amt} C-Coin berhasil!`, "success"); setBidAmt(0); refetch(); }
    catch(e:any){ push(e.message,"error"); } finally{ setBusy(false); }
  }
  async function onBuyNow(){
    if(!user){ push("Login dulu","info"); return; }
    setBusy(true);
    try{ await api.buyNow(listing.id); push("Buy-now berhasil — kartu pindah ke koleksimu!","success"); refetch(); }
    catch(e:any){ push(e.message,"error"); } finally{ setBusy(false); }
  }
  async function onAccept(){
    setBusy(true);
    try{ await api.acceptBid(listing.id); push("Lelang di-settle — ownership pindah, revenue split dieksekusi.","success"); refetch(); }
    catch(e:any){ push(e.message,"error"); } finally{ setBusy(false); }
  }
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:16}}>
    ...
    <div className="grid-2" style={{alignItems:"start"}}>
      <div className="card card-pad" style={{display:"flex",flexDirection:"column",gap:12}}>
        ...
        <div style={{fontSize:18,fontWeight:800}}>{drop?.title ?? card?.id} · Unit #{card?.unitNumber} · {card?.variant}</div>
        ...
        <div style={{fontSize:11,background:"var(--bg-elevated)",border:"1px solid var(--border)",borderRadius:8,padding:8,color:"var(--muted)"}}>
          Fee secondary 15%: 7.5% platform + 7.5% royalty kreator lifetime + 85% seller. Settlement potong C-Coin, seller auto-disburse IDR (fee payout 1%).
        </div>
        {listing.status==="settled" ? <div className="pill pill-success" style={{justifyContent:"center",padding:10}}>✓ Settled — ownership pindah</div>
        : !isAuction ? <button className="btn-gold" onClick={onBuyNow} disabled={busy || isSeller || listing.status!=="listed"} style={{padding:12, opacity: (busy||isSeller)?0.5:1}}>{isSeller ? "Ini listing kamu" : `Buy Now — ${listing.priceCCoin} C-Coin`}</button>
        : <div style={{display:"flex",flexDirection:"column",gap:8}}>
            <div style={{display:"flex",gap:8}}>
              <input className="input" type="number" placeholder={`Min ${minBid} C`} value={bidAmt||""} onChange={e=> setBidAmt(Number(e.target.value))} style={{flex:1}}/>
              <button className="btn-gold" onClick={onBid} disabled={busy} style={{padding:"10px 18px"}}>{busy?"...":"Bid"}</button>
            </div>
            <div style={{fontSize:11,color:"var(--dim)"}}>Min increment 5% → min bid {minBid} C · Anti-sniping: bid di 5 menit terakhir extend +5m</div>
            {isSeller && listing.currentBidCCoin && <button className="btn-ghost" onClick={onAccept} disabled={busy} style={{padding:10}}>Accept Winning Bid — Settle Lelang</button>}
          </div>}
      </div>
      ...
      <div className="card">
        ...
        {bids.length===0 ? <div style={{padding:20,textAlign:"center",color:"var(--muted)",fontSize:13}}>Belum ada bid. Jadilah penawar pertama!</div> :
        <div className="table-wrap">
          <table>
            <thead><tr><th>Bidder</th><th>Jumlah</th><th>Waktu</th></tr></thead>
            <tbody>
              {bids.map((b:any)=> <tr key={b.id}>
                <td style={{fontWeight:600}}>{b.bidderName} {b.bidderId===listing.currentBidderId && <span className="pill pill-success" style={{marginLeft:6,fontSize:10}}>TOP</span>}</td>
