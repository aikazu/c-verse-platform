...
export default function DropDetail(){
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:18}}>
    <Link to="/drops" style={{fontSize:13,color:"var(--muted)"}}>← Kembali</Link>
    <div className="grid-2" style={{alignItems:"start"}}>
      <div className="card" style={{overflow:"hidden"}}>
        ...
        <div className="card-pad">
          ...
          <div style={{display:"flex",gap:16,marginTop:14,flexWrap:"wrap"}}>
            <div><div style={{fontSize:11,color:"var(--dim)",fontWeight:700}}>TOTAL</div><div style={{fontWeight:700}}>{drop.totalUnits}</div></div>
            ...
            <div><div style={{fontSize:11,color:"var(--dim)",fontWeight:700}}>HARGA</div><div style={{fontWeight:800}}>{price} C-Coin · <span style={{fontSize:11,color:"var(--muted)"}}>{formatIdr(price*10000)}</span></div></div>
          </div>
          <div className="progress" style={{marginTop:12}}><div className="progress-fill" style={{width:`${pct}%`}}/></div>
          <div style={{fontSize:11,color:"var(--dim)",marginTop:8}}>oleh {drop.creatorName} · {dropAt ? new Date(dropAt).toLocaleString("id-ID") : ""}</div>
      ...
      <div className="card card-pad" style={{display:"flex",flexDirection:"column",gap:14}}>
        <div>
          <div style={{fontWeight:700,fontSize:16}}>Beli Kartu</div>
          <div className="muted" style={{fontSize:12}}>Maksimal 1 kartu per drop</div>
        </div>
        <div style={{background:"var(--bg-elevated)",border:"1px solid var(--border)",borderRadius:10,padding:12,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div><div style={{fontSize:11,color:"var(--dim)",fontWeight:700}}>HARGA</div><div style={{fontWeight:800,fontSize:18}}>{price} C-Coin</div></div>
        </div>
        {!isLive ? <div className="pill pill-warn" style={{justifyContent:"center",padding:"10px"}}>Belum tersedia</div>
        : <Link to={"/drops/"+drop.id+"/checkout"} className="btn-gold" style={{padding:"14px",fontSize:15, width:"100%", textAlign:"center", textDecoration:"none"}}>Beli Sekarang</Link>}
        <div style={{fontSize:11,color:"var(--dim)"}}>
          Saldo kurang? <Link to="/wallet" style={{color:"var(--gold)",fontWeight:700}}>Isi C-Coin →</Link>
        </div>
      </div>
    </div>
  </div>;
