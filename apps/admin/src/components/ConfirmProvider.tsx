...
// Modal konfirmasi in-app (pengganti native window.confirm).
// Styling memakai kelas .cfm-* yang ter-inherit dari apps/web/src/styles.css via @import.
...
export interface ConfirmOptions {
  ...
  confirmLabel?: string;
  cancelLabel?: string;
  /** Aksi irreversible (draw, refund, batal pengiriman) — tombol konfirmasi merah. */
  danger?: boolean;
}
...
export function ConfirmProvider({ children }: { children: React.ReactNode }) {
  ...
  // Escape = batal (aksesibilitas keyboard).
  useEffect(() => {
    if (!request) return;
