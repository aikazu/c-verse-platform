...
> sale: RPC `cancel_seed_sale` di RPC `07`–`17` (sebelumnya
...
> di RPC `07`–`17` (sebelumnya `20260821020000_seed_two_phase.sql`),
...
## 1. Legal — C-Coin/C-Gems (RESOLVED 2026-08-13; amended 2026-09-03)
...
- **Vault-first recommendation**: Barang fisik sebaiknya tetap
  ...
  `05_data_model.md`).
- **AMEND 2026-09-03 — dual-token C-Coin/C-Gems**: keputusan owner
  menuntaskan pemisahan "hasil titip jual" yang sebelumnya nyampur
  dengan saldo belanja di satu ledger. C-Coin = saldo belanja dari
  top-up (one-way, non-cashable, non-refundable — posisi "bukan
  e-money" bertahan). C-Gems = saldo penghasilan yang lahir HANYA
  dari settlement milik sendiri (release seed, seller 85%
  buyout/accept-bid, royalti kreator, Dukungan 100%) —
  non-transferable antar user. Pintu keluar Gems: (1) payout ke
  IDR (KYC, batch mingguan, fee 1%; hasil pencairan dicatat
...
- Badge: definisi (kriteria + ikon + XP reward) dikonfigurasi
  ...
  aktif di `trg_badge_ownership` (RPC `07`–`17`). "exp" =
...
- Bid bisa dibatalkan 24 jam setelah dipasang (BID_CANCEL_COOLDOWN);
  ...
  (07–17 RPC files) dan `BID_CANCEL_COOLDOWN_HOURS` di shared — dipin literal test
...
- [2026-09-03] Sumber dana payout = **C-Gems** (lot matured > 24
  jam, FIFO) — bukan C-Coin. Nilai minimum/fee tidak berubah
  (lihat C-01 amend & D3b).
...
- **PERLUASAN SEED (TERIMPLEMENTASI 2026-08-21, sebelumnya
  `20260821000000_seed_card.sql` — sekarang di RPC `07`–`17`)**:
...
- **Gate vault-in TERIMPLEMENTASI (2026-08-21) — TWO-PHASE SETTLEMENT
  ...
  **PHASE-1 LOCK** (deal terkunci, kartu `bid_pending`, seller belum
  ...
  sekarang di RPC `07`–`17`). **verified hanya
...
- **Admin abort PHASE-1 stuck sale (2026-08-23)**: jika kartu seed
  ...
  revenue leg. RPC `cancel_seed_sale` di RPC `07`–`17` (sebelumnya
...
| Leaderboard punya halaman sendiri (PG-LB-01) — multi-type: `xp` (default), `cards`, `badges`, `creator` (papan per-kreator via `?tab=`); privasi dijamin RPC `get_leaderboard` (RPC `07`–`17`) | FINAL (2026-08-12; expanded 2026-08-27) |
