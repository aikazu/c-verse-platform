...
| PG-BADGE-01 | `/badges` | Kabinet Prestasi | Galeri 43 lencana; delapan jalur x lima tingkat plus tiga khusus. Cari/filter, detail syarat + hadiah XP, contoh Bronze-Silver-Gold-Astral-Nova. Sesi pemilik membuka progres privat dan XP snapshot yang sudah diraih. Link dari Leaderboard/menu; definisi nonaktif hanya terlihat jika sudah diraih. Komponen detail juga dipakai langsung pada profil. |
...
| PG-PROF-01 | `/u/:username` | Profil kolektor (publik) | Koleksi, level, badge, ranking leaderboard — **kecuali user mengaktifkan privacy anonymous ATAU di-suspend (`flag_reason`)**. Klik lencana membuka pop-up detail di profil: lambang, tingkat, syarat, XP snapshot, dan tanggal perolehan milik kolektor. Tutup/Escape mengembalikan fokus ke lencana. |
...
| PG-USR-05 | `/wallet` | Wallet C-Coin & C-Gems | Saldo belanja C-Coin dan penghasilan C-Gems, mutasi ledger, histori top-up, konversi Gems→C-Coin, serta status payout; **top-up di sini (bukan halaman publik)** |
