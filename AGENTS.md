...
- Public assets: R2 `cverse-assets` is live in APAC at `https://assets.c-verse.co` (`r2.dev` off); `supabase/seed-assets.json` maps six verified fixture objects and `pnpm seed:assets` remains read-only. Upload routes/UI may change the URL mapping only through the API; KYC stays in the separate private `cverse-kyc` bucket. Never seed mock users or move fixture URLs on a live production DB.
